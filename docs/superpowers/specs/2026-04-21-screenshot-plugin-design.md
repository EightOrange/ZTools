...
| 维度     | 决定                                | 理由                                   |
| -------- | ----------------------------------- | -------------------------------------- |
| 插件类型 | 内置无头插件（类似 system 插件）    | 截图 UI 在独立覆盖窗口，不需要插件视图 |
...
| 数据传输 | 临时文件（非 data URL）             | 4K 截图 data URL 过大，临时文件更高效  |
| IPC 方式 | 扩展现有 preload.js                 | 保持一致性，避免额外 preload 文件      |

### 审查修订记录（v2）

| 编号 | 问题                                                    | 修复                            |
| ---- | ------------------------------------------------------- | ------------------------------- |
| #1   | Windows 透明窗口应使用 backgroundColor 而非 transparent | 覆盖窗口按平台区分透明处理      |
| #2   | 插件架构冲突：不应设 main                               | 改为无头插件，无 main 字段      |
| #3   | IPC 通道未在 preload 暴露                               | 在 preload.js 添加截图相关方法  |
| #4   | DPI 缩放未处理                                          | 全流程乘以 display.scaleFactor  |
| #5   | data URL 传输性能差                                     | 改用临时文件传输截图数据        |
| #6   | fullscreen 过渡动画                                     | 不用 fullscreen，用 bounds 模拟 |
| #7   | 多显示器 source 匹配脆弱                                | 用 source.display_id 精确匹配   |
| #8   | Fabric.js v6 API 待验证                                 | 实现前查阅官方文档              |
| #9   | 缺少 logo.png                                           | 创建简易 SVG/PNG logo           |
| #10  | 快捷键冲突未处理                                        | 注册失败时回退或提示用户        |
